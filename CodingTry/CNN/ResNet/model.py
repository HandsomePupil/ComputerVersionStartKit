...
# 此Block为ResNet-18，34layer 的标准模块
class BasicBlock(nn.Module):
    # 定义expansion为了模块化处理卷及操作，Reset-18，34layer 的标准模块中每个layer_Conv内通道数不变，因此保持1不变
    expansion = 1
    
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, **kwargs) -> None:
        super(BasicBlock, self).__init__()
        #全描述与省略的两种不同写法,有时候还是全描述更便于理解与阅读
        ...
        self.downsample = downsample    #定义一个默认为None的参数，在_make_layer函数处控制标准模块(BasicBlock)与瓶颈模块(Bottleneck)的下采样
        
    def forward(self, x):
        identity = x
        # 如果需要做下采样，就加入下采样后进行残差融合
...
# 此Block为ResNet-50，101,152 layer 的标准模块
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, groups=1, width_per_group=64) -> None:
        ...
        #这样写只是方便模型迁移,只需要知道瓶颈模块第三个卷积输出的维度是第一个和第二个的{expansion = 4 }倍
...
class ResNet(nn.Module):
    def __init__(self, block, blocks_num, num_classes=5,include_top=True, groups=1, width_per_group=64) -> None:
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        #  此处错写为了if (stride != 1) or (self.in_channel != channel*block.expansion):导致一直Run不起来，后来发现or and 的用法其实大有玄机
